...
void utf32_to_utf8_mmc(const char32_t* src, size_t src_len, char* dst)
{
...
void utf16_to_utf8_mmc(const char16_t* src, size_t src_len, char* dst)
{
    if (src == NULL || src_len == 0 || dst == NULL) {
        return;
    }
...
ssize_t utf8_length_mmc(const char *src)
{
    const char *cur = src;
    size_t ret = 0;
    while (*cur != '\0') {
...
void utf8_to_utf32_mmc(const char* src, size_t src_len, char32_t* dst)
{
    if (src == NULL || src_len == 0 || dst == NULL) {
        return;
    }
...
void utf8_to_utf16_mmc(const uint8_t* u8str, size_t u8len, char16_t* u16str) {
