...
static jobject createParcelFileDescriptor(JNIEnv * env, int fd)
{
    ...
    jobject parcelFileDesc = android::newParcelFileDescriptor(env, fileDesc);
...
int register_com_sqlcrypt_database_SQLiteStatement(JNIEnv * env)
{
    jclass clazz;

    clazz = env->FindClass("com/sqlcrypt/database/sqlite/SQLiteStatement");
    if (clazz == NULL) {
        LOGE("Can't find com/sqlcrypt/database/sqlite/SQLiteStatement");
        return -1;
    }
    ...
    return android::AndroidRuntime::registerNativeMethods(env,
        "com/sqlcrypt/database/sqlite/SQLiteStatement", sMethods, NELEM(sMethods));
}
