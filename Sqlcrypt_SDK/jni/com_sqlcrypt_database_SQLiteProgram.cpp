...
{
    char buf[65];
    strcpy(buf, "com_sqlcrypt_database_SQLiteProgram->native_compile() not implemented");
...
int register_com_sqlcrypt_database_SQLiteProgram(JNIEnv * env)
{
    jclass clazz;

    clazz = env->FindClass("com/sqlcrypt/database/sqlite/SQLiteProgram");
    if (clazz == NULL) {
        LOGE("Can't find com/sqlcrypt/database/sqlite/SQLiteProgram");
        return -1;
    }
    ...
    return android::AndroidRuntime::registerNativeMethods(env,
        "com/sqlcrypt/database/sqlite/SQLiteProgram", sMethods, NELEM(sMethods));
}
